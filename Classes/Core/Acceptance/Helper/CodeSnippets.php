 ...
 */
class CodeSnippets extends Module
{
    protected $config = [
        'sourcePath' => '',
    ...
    {
        return FileUtility::getPathBySegments($this->_getConfig('targetPath'), $filePath . '.rst.txt');
    }

    protected function getRelativeSourcePath(string $filePath): string
    {
        return FileUtility::getPathBySegments($this->_getConfig('sourcePath'), $filePath);
    }

    protected function getAbsoluteTypo3Path(string $relativePath): string
    {
        return FileUtility::getPathBySegments(Environment::getPublicPath(), $relativePath);
    }

    protected function getAbsoluteDocumentationPath(string $relativePath): string
    {
        return FileUtility::getPathBySegments(
            $this->getScreenshots()->_getConfig('basePath'),
            $this->getScreenshots()->_getConfig('documentationPath'),
            $relativePath
        );
    }
    ...
    protected function readJson(string $path, array $fields, int $inlineLevel): string
    {
        ...
        $code = JsonUtility::extractFieldsFromJson($json, $fields, $inlineLevel);
        return $code;
    }
    ...
    protected function readPhpArray(string $path, array $fields): string
    {
        ...
        if (empty($fields)) {
            ...
            $phpArray = ArrayUtility::extractFieldsFromArray($phpArray, $fields);
            $code = sprintf("'%s' => %s\n",
    ...
    {
        return ClassUtility::extractMembersFromClass($class, $members, $withComment);
    }

    protected function readXml(string $path, array $nodes): string
    {
        $xml = file_get_contents($path);
        $code = XmlUtility::extractNodesFromXml($xml, $nodes);
        return $code;
    }
    ...
    protected function readYaml(string $path, array $fields, int $inlineLevel): string
    {
        ...
        $code = YamlUtility::extractFieldsFromYaml($yaml, $fields, $inlineLevel);
        return $code;
    }
    ...
    protected function write(
        ...
        $code = StringUtility::indentMultilineText($code, '   ');
...
NOWDOC;
    ...
    public function getScreenshots(): Screenshots
    {
        return $this->getModule(Screenshots::class);
    }
}
